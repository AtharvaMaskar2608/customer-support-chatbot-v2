...
Benchmark RRF-only hybrid retrieval (dense vector + full-text arms fused by Reciprocal Rank Fusion) over the `kb_faq` knowledge base, isolating pure retrieval-infrastructure latency from query-embedding latency, capturing the executed query plan, and persisting reproducible latency/throughput metrics read-only against the KB.
