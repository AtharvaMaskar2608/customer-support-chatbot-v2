...
Generate a repeatable golden set for retrieval-quality evaluation of the RAG pipeline from the real `kb_faq` knowledge base, using DeepEval's `Synthesizer` with Claude. Because each `kb_faq.chunk` embeds its FAQ question verbatim, the harness produces paraphrased synthetic queries (not verbatim copies) that keep the source chunk as the ground-truth retrieval target, plus a small raw-question baseline set to quantify the circularity gap, and persists both datasets read-only against the knowledge base.
